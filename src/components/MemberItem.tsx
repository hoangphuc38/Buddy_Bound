...
const MemberItem = ({ press, horizontal, item, isInvited }: IMemberItem) => {
    const getItemUserData = (item: TMember | TUser) => {
      if ('user' in item) {
        return { fullName: item.user.fullName, avatar: item.user.avatar };
      }
      return { fullName: item.fullName, avatar: item.avatar };
    };
        ...
        return (
          <View
            ...
            <TouchableOpacity onPress={press} className={`px-4 py-2 ${isInvited ? 'bg-secondary' : 'bg-primary'} rounded-full`}>
              <Text className="text-white font-interBold">{!isInvited ? 'Invite' : 'Invited'}</Text>
            </TouchableOpacity>
          </View>
        );
      } else {
